...
load("@com_nvidia_isaac//engine/build:template.bzl", "template_rule")
...
cc_library(
    ...
    deps = select({
        "@com_nvidia_isaac//engine/build:platform_jetpack43": [":simd_aarch64"],
        "@com_nvidia_isaac//engine/build:platform_x86_64": [":simd_x86_64"],
    }),
)
...
genrule(
    ...
    outs = ["jconfig.h"],
    cmd = select({
        "@com_nvidia_isaac//engine/build:platform_jetpack43": "cp $(location jconfig_nowin_simd.h) $@",
        "@com_nvidia_isaac//engine/build:platform_x86_64": "cp $(location jconfig_nowin_simd.h) $@",
    }),
)
