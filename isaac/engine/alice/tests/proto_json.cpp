...
// Keep plan frame changing so that Plan2Generator keeps publishing messages.
// Ohterwise, Plan2Generator publishes plan once not to repeat the same message.
class PoseChanger : public Codelet {
 public:
  void start() override {
    tickPeriodically();
  }
  void tick() override {
    // Change world_T_plan randomly. See codelet description.
    const Pose2d pose = Pose2d::Rotation(WrapTwoPi(getTickTime()));
    node()->pose().set("world", "plan", pose, getTickTime());
  }
};
...
ISAAC_ALICE_REGISTER_CODELET(isaac::alice::PoseChanger);
