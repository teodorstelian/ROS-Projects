...
TEST(Config, PrefixedConfigFile) {
  constexpr char app_json_text[] =
      R"???({
  "name": "ConfigGetAllJson",
  "config_files": ["my_prefix:engine/alice/tests/config_1.config.json"],
  "config": {
    "my_prefix.test node": {
      "test component": {
        "foo": "hola",
  ...
  Node* node = app.createNode("my_prefix.test node");
  auto* codelet = node->addComponent<MyConfigTest>("test component");

  EXPECT_STREQ(codelet->get_foo().c_str(), "hola2");
  EXPECT_EQ(codelet->get_bar(), 38);
  EXPECT_EQ(codelet->get_pie(), 1.4153);
  EXPECT_TRUE(codelet->get_xin());
}
