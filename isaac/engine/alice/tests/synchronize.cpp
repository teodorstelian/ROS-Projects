...
class MySubscriber : public Codelet {
 public:
  void start() override {
    ...
    for (int i = 0; i < get_num_repeat_synchronize(); i++) {
  ...
  // How many times synchronize is called. This checks that calling synchronize multiple times
  // is not an error.
  ISAAC_PARAM(int, num_repeat_synchronize, 1);
};
...
class SynchronizeMany : public Codelet {
 public:
  void start() override {
    synchronize(rx_foo1(), rx_foo2());
    synchronize(rx_foo3(), rx_foo4());
    synchronize(rx_foo5(), rx_foo6());
    synchronize(rx_foo7(), rx_foo8());
    synchronize(rx_foo2(), rx_foo3());
    synchronize(rx_foo6(), rx_foo7());
...
// Receives and synchronizes many message streams.
class SynchronizeGroups : public Codelet {
 public:
  void start() override {
    // Make sure adding several channel works for 3
    synchronize(rx_foo1(), rx_foo2(), rx_foo3());
    // and higher number...
    synchronize(rx_foo4(), rx_foo5(), rx_foo6(), rx_foo7(), rx_foo8(), rx_foo1());
    tickOnMessage(rx_foo1());
  }
  void tick() override {
    ASSERT_TRUE(rx_foo1().available());
...
TEST(MessagePassing, Test1) {
  for (int i = 1; i <= 3; i++) {
    ...
    sub->async_set_num_repeat_synchronize(i);
...
void RunTestForMany() {
  ...
  Node* sub_node = app.createMessageNode("sub");
  auto* sub = sub_node->addComponent<T>();
...
TEST(MessagePassing, SynchronizeMany) {
  RunTestForMany<SynchronizeMany>();
}

TEST(MessagePassing, SynchronizeGroups) {
  RunTestForMany<SynchronizeGroups>();
}
...
ISAAC_ALICE_REGISTER_CODELET(isaac::alice::SynchronizeGroups);
