...
class Application {
 public:
  ...
  void startWaitStop();
  // Starts the app, wait for node succeed/fail if provided. Expire after max_duration (seconds) if
  // it is positive value. Ctrl-C/SIGINT is honored.
  void run(const std::optional<double> max_duration, const std::optional<std::string> node_name);
