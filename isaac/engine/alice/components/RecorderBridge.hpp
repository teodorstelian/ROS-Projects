...
class RecorderBridge : public alice::Codelet {
 ...
 private:
  // Sends reply to the Front End
  void sendNotice(const std::string& cmd, const std::string& error_msg);
  // Validates an incomming msg from the Front End
  bool isMessageValid(const Json& json, const std::string& expected_cmd,
      const std::string& expected_param);
  // Creates a unique folder using a tag as prefix
  std::string getSubFolder() const;
  // Returns the full path folder base_name_/app_uuid_/tag_##id.
  std::string getFullPath() const;
  // Validates the desired log path
