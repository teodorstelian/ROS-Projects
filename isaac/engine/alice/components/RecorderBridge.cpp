...
void RecorderBridge::tick() {
  ...
  if (!recorder_component_) {
    ...
    if (recorder_component_->recordingEnabled()) {
      records_number_ = 1;
  ...
  rx_request().processAllNewMessages([this](auto json, int64_t pubtime, int64_t acqtime) {
    if (this->recorder_component_->numChannels() == 0) {
    ...
    } else if (this->recorder_component_->recordingEnabled()) {
      if (this->isMessageValid(json, kRequestRecCmd, kStopParam)) {
        // Stop recording logic
        this->recorder_component_->stopRecording();
      ...
      if (this->isMessageValid(json, kRequestRecCmd, kStartParam)) {
        ...
        this->recorder_component_->async_set_tag(this->getSubFolder());
        if (this->validatePath()) {
          this->recorder_component_->startRecording(
              this->base_path_, this->getSubFolder());
          ...
          const std::string root = this->getFullPath();
...
std::string RecorderBridge::getFullPath() const {
  return base_path_ + "/" + app_uuid_ + "/" + getSubFolder();
}

bool RecorderBridge::validatePath() const {
  // Validate that you can write in base path and that root does not exist
  if (!IsValidDirectory(base_path_)) {
    std::experimental::filesystem::create_directories(base_path_);
  }
  return IsValidWriteDirectory(base_path_) && !IsValidDirectory(getFullPath());
}

std::string RecorderBridge::getSubFolder() const {
  std::stringstream number_formater;
  number_formater << tag_ << std::setw(get_count_width()) << std::setfill('0') << records_number_;
