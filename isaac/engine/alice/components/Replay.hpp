...
class Replay : public Codelet {
 public:
  ...
  // Starts replay of a loaded log with a given offset.
  void startReplay(int64_t replay_time_offset);
  // Stops an ongoing replay
  void stopReplay();
  // Opens cask from root and parses contents without starting replay
  void loadLog(const std::string& cask_directory);
  ...
  // If enabled automatically stops the application when the log was fully replayed
  ISAAC_PARAM(bool, auto_stop_application, false);
  // If enabled sets node status to: success the log was fully replayed, failure in case or errors
...
ISAAC_ALICE_REGISTER_CODELET(isaac::alice::Replay)
