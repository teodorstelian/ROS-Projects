...
void Replay::loadLog(const std::string& cask_directory) {
  ...
  set_cask_directory(cask_directory);
  readChannelIndex();
...
void Replay::start() {
  startReplay(get_replay_time_offset());
  // Set tick_period, unless specified by user.
...
void Replay::initialize() {
  ...
  message_ledger_->addOnConnectAsTxCallback(
      [this](const MessageLedger::Endpoint& tx, const MessageLedger::Endpoint& rx) {
        ...
        message_ledger_->addOnMessageCallback(
            tx, rx.component, [this, rx](ConstMessageBasePtr message) {
...
void Replay::startReplay(int64_t replay_time_offset) {
  std::lock_guard<std::recursive_mutex> lock(mutex_);
  set_replay_time_offset(replay_time_offset);
  if (!cask_) {
    loadLog(get_cask_directory());
  }
  if (!cask_) {
    return;
  }
...
void Replay::readChannelIndex() {
  std::vector<CaskMessageChannelEntry> indices;
  FromCask(*cask_, indices);

  for (const auto& index : indices) {
    const std::string tag = index.tag;
    const Uuid series_uuid = index.series_uuid;
...
void Replay::readChannelMessageHeaders(const std::string& tag) {
  ...
  channel_replay.history = std::move(series.history);

  const auto& history = channel_replay.history;
  if (history.empty()) {
    LOG_WARNING("Replay '%s' has no message", tag.c_str());
    return;
  }
  ...
  if (get_replay_time_offset() < time_range_.start) {
    set_replay_time_offset(time_range_.start);
  }
...
void Replay::enqueueNextMessage(const std::string& tag, int64_t prev_timestamp) {
  ...
  job_descriptor.groups.push_back(node()->name());
