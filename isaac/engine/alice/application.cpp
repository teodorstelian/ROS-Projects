...
void Application::run(const std::optional<double> max_duration,
                      const std::optional<std::string> node_name) {
  // Tries to find specified node
  const Node* node = nullptr;
  if (node_name) {
    node = backend_->node_backend()->findNodeByName(*node_name);
  }
  ...
  if (node == nullptr && max_duration) {
    clock->sleep(*max_duration);
  } else {
    // Sleep until interruption is requested
    // TODO use a conditional variable or such
    while (is_running_) {
      if ((node != nullptr && node->getStatus() != Status::RUNNING) ||
          (max_duration && clock->time() > *max_duration)) {
        break;
      }
      clock->sleep(0.1);
    }
