...
load("//engine/build/sphinx:sphinx.bzl", "sphinx", "sphinx_dep")
...
def isaac_cc_module(
    ...
    native.cc_binary(
        ...
        deps = [Label("//engine/alice/tools:module")] + deps + [modlib_name],
        tags = tags,
        linkshared = True,
        **kwargs
    )
...
def isaac_component(name, filename = None, deps = [], visibility = [], **kwargs):
    ...
    isaac_cc_library(
        ...
        visibility = visibility,
        alwayslink = True,
        deps = deps + [Label("//engine/alice"), Label("//engine/core"), Label("//messages")],
        **kwargs
    )
...
_isaac_app_runscript = rule(
    ...
    attrs = {
        ...
        "template": attr.label(
            default = Label("//engine/alice/tools:run.sh.tpl"),
            allow_single_file = True,
        ),
    },
...
def isaac_graph(name, graph = None, modules = [], data = [], **kwargs):
    ...
    native.cc_test(
        ...
        deps = [Label("//engine/alice/tools:check_json")],
...
def isaac_app(name, modules = [], app_json_file = None, data = [], script = None, tags = [], excludes = [], remap_paths = {}, **kwargs):
    ...
    native.sh_binary(
        ...
        name = name,
        srcs = [script],
        data = [Label("//engine/alice/tools:main")] + [app_json_file] + module_sos + data,
        **kwargs
    )
...
def isaac_pkg(name, srcs, data = [], remap_paths = {}, excludes = [], **kwargs):
    ...
    '''
    remap_paths_copy = {"engine/build/run": "run"}
    remap_paths_copy.update(remap_paths)
    _isaac_pkg(
        name = name,
        srcs = srcs + [Label("//engine/build:run_script")],
        data = data,
...
def _yield(repo_rule, name, **kwargs):
    """
    Helper function to only claim external repo when the name is not claimed yet
    """
...
def isaac_py_app(name, modules = [], data = [], srcs = [], tags = [], excludes = [], remap_paths = {}, **kwargs):
    '''
...
_isaac_jupyter_runscript = rule(
    ...
    attrs = {
        ...
        "template": attr.label(
            default = Label("//engine/pyalice:jupyter_launcher.sh.tpl"),
            allow_single_file = True,
        ),
    },
...
def isaac_jupyter_app(name, notebook, modules = [], data = [], excludes = [], tags = [], remap_paths = [], **kwargs):
    ...
    native.sh_binary(
        name = name,
        srcs = [script],
        data = [Label("//engine/pyalice"), notebook] + module_sos + data,
        tags = tags,
        **kwargs
    )
