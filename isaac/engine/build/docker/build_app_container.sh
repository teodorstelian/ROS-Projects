...
function show_help
{
    ...
    echo "Arguments:"
    echo "  -p <platform>    The platform to build for (valid choices: x86_64 (default), jetpack43)"
...
function build_single_app
{
    ...
    docker run \
           ...
           -w /src/workspace \
           isaacbuild \
           ./engine/build/deploy.sh ${CACHEARG} -p "${APP}-pkg" -d "${PLATFORM}" -h localhost --deploy_path "${TMPDIR}/app"
...
function build_docker_file_jetpack
{
    ...
    cp engine/build/scripts/install_dependencies_jetson.sh "${DIRECTORY}"
