...
TEST(Serialization, CompositeQuantityFromJson) {
  using T = composite::Quantity;
  const T val{"entity", composite::Measure::kPosition, VectorXi::Constant(1, 3)};

  Json json = Json::array({"entity", "position", Json::array({3})});
  auto maybe = TryGet<T>(json);
  ASSERT_TRUE(maybe);
  EXPECT_EQ(maybe->entity, val.entity);
  EXPECT_EQ(maybe->measure, val.measure);
  EXPECT_EQ(maybe->dimensions, val.dimensions);
}

TEST(Serialization, CompositeQuantityToJson) {
  using T = composite::Quantity;
  const T val{"entity", composite::Measure::kRotation, VectorXi::Constant(1, 4)};
...
TEST(Serialization, CompositeSchemaFromJson) {
  using T = composite::Schema;
  const T val{{"shoulder", "elbow", "wrist"}, composite::Measure::kAngularSpeed};

  Json json;
  json["entity"] = Json::array({"shoulder", "elbow", "wrist"});
  json["measure"] = "angularSpeed";

  auto maybe = TryGet<T>(json);
  ASSERT_TRUE(maybe);
  EXPECT_EQ(maybe->getElementCount(), val.getElementCount());

  const auto val_q = val.getQuantities();
  const auto tag_q = maybe->getQuantities();
  for (size_t i = 0; i < val_q.size(); i++) {
    EXPECT_EQ(tag_q[i].entity, val_q[i].entity);
    EXPECT_EQ(tag_q[i].measure, val_q[i].measure);
    EXPECT_EQ(tag_q[i].dimensions, val_q[i].dimensions);
  }
}

TEST(Serialization, CompositeSchemaFromJsonQuantityList) {
  using T = composite::Schema;
  const T val{{"shoulder", "elbow", "wrist"}, composite::Measure::kAngularSpeed};

  Json json;
  Set(json, val.getQuantities());
...
TEST(Serialization, CompositeSchemaToJson) {
  using T = composite::Schema;
  const T val{{"shoulder", "elbow", "wrist"}, composite::Measure::kAngularSpeed};
