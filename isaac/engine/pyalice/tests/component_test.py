...
import uuid
import numpy as np
from engine.pyalice import Application, Cask, Message, Node, Component
...
class TestComponent(unittest.TestCase):
    ...
    def test_component_config(self):
        ...
        component = node.add(self._app.registry.isaac.viewers.ColorCameraViewer)
    ...
    def test_component_access(self):
        ...
        result = self._app.load_module('viewers')
        self.assertTrue(result)
        node.add(self._app.registry.isaac.viewers.ColorCameraViewer)

        component = node.components.ColorCameraViewer
        self.assertTrue(isinstance(component, Component.Component))
        self.assertIsNotNone(component)

        self.assertEqual(node.components.ColorCameraViewer.config['target_fps'], 30.0)
        ...
        node.components.ColorCameraViewer.config['target_fps'] = 45.0
        self.assertEqual(node.components['ColorCameraViewer'].config['target_fps'], 45.0)
