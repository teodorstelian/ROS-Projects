...
from engine.pyalice import Application, Cask, Message, Node, Status, Codelet
...
class TestApplication(unittest.TestCase):
    ...
    def test_load_subgraph(self):
        ...
        self._app.load('engine/pyalice/tests/application_test.subgraph.json', 'foo')

        node = self._app.nodes['foo.camera_viewer']
        component = node['ColorCameraViewer']
        self.assertIsNotNone(component)

        fps_readback = self._app.nodes['foo.camera_viewer'].components['ColorCameraViewer'].config[
            'target_fps']
        self.assertEqual(fps_readback, 11)

        camera_name_readback = self._app.nodes['foo.camera_viewer'].components[
            'ColorCameraViewer'].config['camera_name']
        self.assertEqual(camera_name_readback, 'bar')

        # loads the subgraph again with different prefix name
        self._app.load('engine/pyalice/tests/application_test.subgraph.json', 'foo1')
        node = self._app.nodes['foo1.camera_viewer']
        component = node['ColorCameraViewer']
        self.assertIsNotNone(component)

        fps_readback = self._app.nodes['foo1.camera_viewer'].components['ColorCameraViewer'].config[
            'target_fps']
        self.assertEqual(fps_readback, 11)
    ...
    def test_load_subgraph_in_asset_path(self):
        ...
        self._app._asset_path = 'engine/pyalice/tests/mock_asset_path'
        ...
        component = pynode['ColorCameraViewer']
        self.assertIsNotNone(component)
    ...
    def test_module_explorer(self):
        self._app = Application()
        self._app.registry.isaac.alice.JsonToProto
    ...
    def test_node_accesor(self):
        ...
        self.assertIsNotNone(node)
        component = node.add(self._app.registry.isaac.alice.JsonToProto, 'bar1')
        self.assertIsNotNone(component)

        node = self._app.add('foo2')
        self.assertIsNotNone(node)
        component = node.add(self._app.registry.isaac.alice.ProtoToJson, 'bar2')
        self.assertIsNotNone(component)
