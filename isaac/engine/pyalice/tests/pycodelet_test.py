...
class MyPyCodeletProducer(Codelet):
    ...
    def tick(self):
        ...
        epsilon = 0.1
        assert abs(self.tick_time - desired_tick_time) < epsilon, "{}".format(self.tick_time -
                                                                              desired_tick_time)
...
class PyCodeletImageReader(Codelet):
    ...
    def tick(self):
        ...
        assert msg.proto.image.rows == 4096, "Incorrect image rows {}".format(msg.proto.image.rows)
        assert msg.proto.image.cols == 4096, "Incorrect image cols {}".format(msg.proto.image.cols)
        buffer_data = msg.buffers[msg.proto.image.dataBufferIndex]
...
def main():
    # PyCodlet => PyCodelet
    app = Application("engine/pyalice/tests/pycodelet_test_py2py.app.json")
    app.nodes['py_producer'].add(MyPyCodeletProducer)
    app.nodes['py_consumer'].add(MyPyCodeletConsumer)
    app.start_wait_stop(1.0)

    # C++ Codelet => PyCodelet
    app = Application("engine/pyalice/tests/pycodelet_test_cpp2py.app.json")
    app.nodes['py_consumer'].add(MyPyCodeletConsumer)
    app.start_wait_stop(1.0)

    # PyCodelet => C++ Codelet
    app = Application("engine/pyalice/tests/pycodelet_test_py2cpp.app.json")
    app.nodes['py_producer'].add(MyPyCodeletProducer)
    app.start_wait_stop(1.0)

    # Synchronized Tick
    app = Application("engine/pyalice/tests/pycodelet_test_sync.app.json")
