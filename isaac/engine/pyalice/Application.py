...
class _NodesAccess(object):
    ...
    def __init__(self, app):
        ...
        super().__setattr__(self._APP_ATTR_NAME, app)

    def __getattr__(self, name):
        '''
        Gets the node from managed app with given name

        Args:
            name (str): parameter name
        '''
        app = super().__getattribute__(self._APP_ATTR_NAME)
        handle = app._app.find_node_by_name(name)
...
class Application(object):
    ...
    def load(self, filename: str, prefix: str = '') -> bool:
        ...
        if not os.path.isfile(filename):
            ...
            if os.path.isfile(os.path.join(self._asset_path, filename)):
                # Found file in asset path, use this location
                filename = os.path.join(self._asset_path, filename)
            else:
                # Still could not find file, raise exception
                raise Exception('Could not load "{}"'.format(filename))

        return self._app.load(filename, prefix)
    ...
    def run(self, event=None):
        ...
        Args:
            event (str or float or None): name of node to wait for or number of seconds to wait.
                In case of None, would wait for Ctrl-C/SIGINT.
        '''
        if event is not None and not isinstance(event, (float, str)):
        ...
        elif isinstance(self._args.max_duration, float):
            max_duration = min(max_duration, self._args.max_duration)
