...
void PybindApplication::run(pybind11::object max_duration, pybind11::object node_name) {
  std::optional<double> max_duration_cpp =
      max_duration.is_none() ? std::nullopt : std::optional<double>(max_duration.cast<double>());
  std::optional<std::string> node_name_cpp =
      node_name.is_none() ? std::nullopt
                          : std::optional<std::string>(node_name.cast<std::string>());

  {
    pybind11::gil_scoped_release release_gil;
    app_->run(max_duration_cpp, node_name_cpp);
  }
}
...
PybindMessage PybindApplication::receive(const std::string& node_name,
  ...
  MessageBasePtr message_ptr = nullptr;
  ledger->readLatestNew(endpoint, [&message_ptr](const ConstMessageBasePtr& msg) {
    message_ptr = std::const_pointer_cast<isaac::alice::MessageBase>(msg);
  });
...
bool PybindApplication::publish(const std::string& node_name, const std::string& component_name,
  ...
  MessageBasePtr message_base = CreateProtoMessage(uuid, type_id, proto_bytes, cpu_buffers);
